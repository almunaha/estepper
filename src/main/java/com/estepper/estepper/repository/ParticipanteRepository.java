...
public interface ParticipanteRepository extends JpaRepository<Participante, Integer> {
    ...
    @Query("UPDATE Participante p SET p.edad = :edad, p.sexo = :sexo, p.fotoUsuario = :fotoUsuario, p.grupo = :grupo, p.asistencia = :asistencia, p.idCoordinador = :idCoor, p.idAdministrador = :idAdmin, p.perdidaDePeso = :perdidadepeso, p.sesionesCompletas = :sesionescompletas, p.perdidacmcintura = :percmcintura WHERE p.id = :id")
    void update(Integer edad, Sexo sexo, String fotoUsuario, Grupo grupo, Integer asistencia, Integer idCoor,Integer idAdmin, Double perdidadepeso, Integer sesionescompletas, Double percmcintura, Integer id);
